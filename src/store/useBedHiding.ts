...
interface BedHidingState {
  ...
  hideInBed: (bedName: string, playerPosition: [number, number, number], bedPosition: [number, number, number]) => void;
  standUp: () => void;
...
export const useBedHiding = create<BedHidingState>((set) => ({
  ...
  hideInBed: (bedName, playerPosition, bedPosition) => set({ 
    isHiding: true, 
    nearBed: bedName,
  ...
  clearRestoreFlag: () => set({ shouldRestorePosition: false, originalPosition: null, bedPosition: null }),
  reset: () => set({ isHiding: false, nearBed: null, originalPosition: null, bedPosition: null, shouldRestorePosition: false }),
}));
